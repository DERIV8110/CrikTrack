...
def load_scoreboard():
        ...
        c.execute('''CREATE TABLE IF NOT EXISTS MATCH(
                  ...
                  BATTEAM TEXT NOT NULL DEFAULT 0,
                  BALLTEAM TEXT NOT NULL DEFAULT 0,
                  TOSSWINNER TEXT NOT NULL DEFAULT 0
        )''')
        conn.commit()
        conn.close()
        conn = sqlite3.connect('match.db')
        ...
        score_page.grid_columnconfigure(6, weight=1)
        ...
        teama_toss_label = customtkinter.CTkLabel(score_page, text=players_entry.get(), font=("Arial", 16, "bold"))
        teama_toss_label.grid(row=0, column=3, pady=15)

        toss_win_label = customtkinter.CTkLabel(score_page, text="Chose to bat", font=("Arial", 16, "bold"))
        toss_win_label.grid(row=0, column=4, pady=15)

                # ── Row 1: Treeview (col 0-4, fixed size) ─────────────────
        frame5 = ttk.Frame(score_page,height=200)
        frame5.grid(row=1, column=0, columnspan=6, padx=10, pady=10)  # ← FIXED: uncommented and added sticky
        
        # ── score_page must let row 1 expand ──────────────────────
        score_page.grid_rowconfigure(1, weight=1)
        score_page.grid_columnconfigure(0, weight=1)

        # ── frame5 row/column config ───────────────────────────────
        frame5.grid_rowconfigure(0, weight=3)   # tree1 row
        frame5.grid_rowconfigure(1, weight=0)   # gap between trees
        frame5.grid_rowconfigure(2, weight=1)   # tree2 row
        frame5.grid_columnconfigure(0, weight=1)
        frame5.grid_columnconfigure(1, weight=0)  # scrollbar column, fixed
        ...
        def refresh_rr_target_etc():
               ...
               conn = sqlite3.connect('batsman.db')
               c = conn.cursor()
               c.execute('SELECT BALLS,RUNS FROM BATSMAN WHERE NAME = ?',(selected_batsman.get(),))
               rr = c.fetchall() # rr = [(balls,run)]
               conn.commit()
               conn.close()
               
               rr_data = rr[0]   # RUN RATE DATA
               rr_value = rr_data[1]/(rr_data[0]/6)
               balls_remain = (int(overs_entry.get())*6)-rr[0][0]   # INNING ENDING DATA

               if len(tree2.get_children()) == 0:
                       #tree2.insert(parent, index, values=(...))   # for adding a NEW row
                        tree2.insert('', 'end', values=(rr_value, 0, 0, balls_remain, 0))
               else:
                        existing_rows = tree2.get_children()
                        row_id = existing_rows[0]
                        tree2.set(row_id, "Current RR", rr_value)
                        tree2.set(row_id, "Target", 'NA')
                        tree2.set(row_id, "RR Required", 'NA')
                        tree2.set(row_id, "Balls Remain", balls_remain)
                        tree2.set(row_id, "SR", 'NA')
               #print(rr_value)

        def add_0_run():
              conn = sqlite3.connect('batsman.db')
              c = conn.cursor()
              c.execute('UPDATE BATSMAN SET RUNS = RUNS+0, BALLS = BALLS + 1 WHERE NAME = ?', (selected_batsman.get(),))
              conn.commit()
              conn.close()
              refresh_rr_target_etc()
        run_0_button = customtkinter.CTkButton(score_page,text="0",command=add_0_run)   
        run_0_button.grid(row=3,column=0)
        def add_1_run():
              conn = sqlite3.connect('batsman.db')
              c = conn.cursor()
              c.execute('UPDATE BATSMAN SET ONES = ONES+1, RUNS = RUNS+1, BALLS = BALLS + 1 WHERE NAME = ?', (selected_batsman.get(),))
              conn.commit()
              conn.close()
              refresh_rr_target_etc()
        ...
        def add_2_run():
              conn = sqlite3.connect('batsman.db')
              c = conn.cursor()
              c.execute('UPDATE BATSMAN SET TWOS = TWOS+1, RUNS = RUNS+2, BALLS = BALLS + 1 WHERE NAME = ?', (selected_batsman.get(),))
              conn.commit()
              conn.close()
              refresh_rr_target_etc()
        run_2_button = customtkinter.CTkButton(score_page,text="2",command=add_2_run)   
        run_2_button.grid(row=2,column=1)
        def add_3_run():
              conn = sqlite3.connect('batsman.db')
              c = conn.cursor()
              c.execute('UPDATE BATSMAN SET THREES = THREES+1, RUNS = RUNS+3, BALLS = BALLS + 1 WHERE NAME = ?', (selected_batsman.get(),))
              conn.commit()
              conn.close()
              refresh_rr_target_etc()
        run_3_button = customtkinter.CTkButton(score_page,text="3",command=add_3_run)   
        run_3_button.grid(row=2,column=2)
        def add_4_run():
              conn = sqlite3.connect('batsman.db')
              c = conn.cursor()
              c.execute('UPDATE BATSMAN SET FOURS = FOURS+1, RUNS = RUNS+4, BALLS = BALLS + 1 WHERE NAME = ?', (selected_batsman.get(),))
              conn.commit()
              conn.close()
              refresh_rr_target_etc()    
        run_4_button = customtkinter.CTkButton(score_page,text="4",command=add_4_run)   
        run_4_button.grid(row=2,column=3)
        def add_5_run():
              conn = sqlite3.connect('batsman.db')
              c = conn.cursor()
              c.execute('UPDATE BATSMAN SET FIVES = FIVES+1, RUNS = RUNS+5, BALLS = BALLS + 1 WHERE NAME = ?', (selected_batsman.get(),))
              conn.commit()
              conn.close()
              refresh_rr_target_etc()
        run_5_button = customtkinter.CTkButton(score_page,text="5",command=add_5_run)   
        run_5_button.grid(row=2,column=4)
        def add_6_run():
              conn = sqlite3.connect('batsman.db')
              c = conn.cursor()
              c.execute('UPDATE BATSMAN SET SIXES = SIXES+1, RUNS = RUNS+6, BALLS = BALLS + 1 WHERE NAME = ?', (selected_batsman.get(),))
              conn.commit()
              conn.close()
              refresh_rr_target_etc()
        run_6_button = customtkinter.CTkButton(score_page,text="6",command=add_6_run)   
        run_6_button.grid(row=2,column=5)
#---------treeview and function for current run rate-----uses data from sqllite----------
#---------run rate and other data fetched each time user presses any button-----so-------
#---------keep this function in other button's function and treeview external------------
#---------stats treeview-----------------------------------------------------------------
        
         # ── Stats treeview (tree2) ─────────────────────────────────
        tree2 = ttk.Treeview(frame5,
                             columns=("Current RR","Target","RR Required","Balls Remain","SR"),
                             show="headings",
                             height=4)
        tree2.heading("Current RR",   text="Current RR")
        tree2.heading("Target",       text="Target")
        tree2.heading("RR Required",  text="RR Required")
        tree2.heading("Balls Remain", text="Balls Remain")
        tree2.heading("SR",           text="SR")
# ADD WICKET (OUT) BUTTON AND FUNCTIONS RELATED TO THEM (01/07/2026)
        tree2.grid(row=2, column=0, sticky="nsew", pady=(5,0))  # ← FIXED: row=2 not row=3
